...
class APITestCase(unittest.TestCase):
    ...
    def get_api_headers(self, username, password):
        return {
            'Authorization': 'Basic' + b64encode(username + ':' + password).encode('utf-8'),
    ...
    def test_bad_auth(self):
        ...
        u = User(email='a316296241@gmail.com', password='admin', confirmed=True,
                 role=r)
        db.session.add(u)
        db.session.commit()
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers('a316296241@gmail.com', 'dog'))
        self.assertTrue(response.status_code == 401)
    ...
    def test_posts(self):
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u = User(email='a316296241@gmail.com', password='admin', confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()

        # 写一篇文章
        response = self.client.post(url_for('api.new_post'),
                                    headers=self.get_auth_header('a316296241@gmail.com', 'admin'),
        ...
        # 获取刚发布的文章
        response = self.client.get(url, headers=self.get_auth_header('a316296241@gmail.com', 'admin'))
